...
namespace LocalisationAnalyser.Tests.Localisation
{
    public class LocalisationFileTests
    {
        ...
        {
            var localisation = await setupFile($@"{SyntaxTemplates.CreateFileHeader(test_namespace)}
...
<newline>
 <newline with leading space>"");
        ...
        private void checkResult(string inner)
        {
            ...
            sb.Append($@"{string.Format(SyntaxTemplates.CreateFileHeader(test_namespace), string.Empty)}
